...
class GridWidget(QWidget):
    ...
    def paintEvent(self, event):
        ...
        cell = self.cell_size
        grid = self.race.teams[0].map_grid
        ...
        for idx, team in enumerate(self.race.teams):
            ...
            color = QColor(0xFF, 0xA5, 0x00) if status["on_fire"] else team_colors[idx]
            if status["in_smoke"]:
...
class RaceGUI(QMainWindow):
    ...
    def create_ui(self):
        ...
        brand.setStyleSheet("color: #6A0DAD; font-style: italic; font-size: 14px;")
    ...
    def update_display(self):
        ...
        for i, team in enumerate(self.race.teams):
            ...
            self.table.setItem(i, 1, QTableWidgetItem(str(team.rescued)))
            self.table.setItem(i, 2, QTableWidgetItem(str(team.steps_taken)))
    ...
    def all_teams_finished(self):
        return all(team.has_won() for team in self.race.teams)

    def next_turn(self):
        if self.race_finished:
            return
        team = self.race.teams[self.race.current_turn]
        if not team.has_won():
            self.pulse_brain(self.race.current_turn)
            rescued = team.step()
            self.race.total_turns += 1
            if not rescued:
                self.race.extra_steps[team.name] += 1
        if team.has_won():
            self.timer.stop()
            self.race_finished = True
            self.show_winner()
            return
        self.race.current_turn = (self.race.current_turn + 1) % len(self.race.teams)
        self.update_display()
